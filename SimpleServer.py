...
HOST = 'Jeff' # socket.gethostname()
IP = '192.168.2.198' # socket.gethostbyname(HOST)
...
class Command():
    def __init__(self,name):
        self.name = name.upper() # uppercase
        # set these later:
    ...
    if not name:
        ...
        for cmd in COMMAND_LIST:
            if not cmd.passive:
                ...
                screen.add(f'{cmd.name:<{margin}} {marker+cmd.description}') # [marker][name + margin] description
...
if __name__ == '__main__':
    ...
    the_raven = poem_extractor.Extractor('the_raven.txt')
